...
IMPLEMENT_CLASS( DotDraw, wxControl )
BEGIN_EVENT_TABLE( DotDraw, wxControl )
        EVT_MOUSE_EVENTS( DotDraw::OnMouseEvent )
        EVT_KEY_DOWN( DotDraw::OnKeyDownEvent )
        EVT_PAINT( DotDraw::OnPaint )
        EVT_SIZE( DotDraw::OnSize )
//        EVT_CHAR( DotDraw::OnCharEvent )
//        EVT_KEY_UP( DotDraw::OnKeyUpEvent )
//        EVT_MOUSE_CAPTURE_LOST( DotDraw::OnMouseCapureLost )
END_EVENT_TABLE ()


DotDraw::DotDraw (wxWindow *parent, wxWindowID id,
	...
	const wxValidator &validator)
{
	...
	gvc = gvContext();
	resGraph = agopen ((char *)"DotGraph", AGRAPHSTRICT);

}

void DotDraw::OnPaint(wxPaintEvent& evt)
{
...
void DotDraw::drawApplication (wxDC &dc)
{

}

void DotDraw::drawPlatform (wxDC &dc)
{
...
void DotDraw::OnSize (wxSizeEvent& evt)
{
	Refresh();
}

void DotDraw::OnMouseEvent(wxMouseEvent& evt)
{
	this->SetFocus();
...
/*	//Selection of states for one-state representation disabled
	if (evt.LeftDown()){
		long x = evt.GetX();
		long y = evt.GetY();
		Dot *node = [current searchWithX: x
		                andY: y
...
void DotDraw::OnKeyDownEvent(wxKeyEvent& evt)
{
...
/*
void DotDraw::highlightDotNode (long x, long y)
{
        if (current){
                ...
                if (node != highlighted){
                        wxPaintDC dc(this);
                        this->unhighlightDotNode(dc);
                        this->drawHighlightDotNode (node, dc);
                        highlighted = node;
                }
        }
}

void DotDraw::unhighlightDotNode (wxDC &dc)
{
		...
		brush = wxBrush (color, wxTRANSPARENT);
                this->drawDotNode (parent, 0, brush, grayColor, dc);
	...
	for (i = 0; i < [[[highlighted parent] aggregatedChildren] count]; i++){
		id agg = [[[highlighted parent] aggregatedChildren] objectAtIndex: i];
        	...
        	this->drawDotNode (agg, 0, brush, grayColor, dc);
	}
}

void DotDraw::drawHighlightDotNode (id node, wxDC &dc)
{
        wxColour blackColor = wxColour (wxT("#000000"));
        wxColour color = this->findColorForNode (node);
        wxBrush brush (color, wxTRANSPARENT);
        this->drawDotNode (node, 1, brush, blackColor, dc);
        ...
        while (parent){
                color = this->findColorForNode (parent);
                this->drawDotNode (parent, 0, brush, blackColor, dc);
...
void DotDraw::drawDotNode (id node, int offset,
...
wxColour DotDraw::findColorForNode (id treemap)
{
        wxColour color;
...
void DotDraw::drawDot (id treemap, wxDC &dc)
{
	if ([treemap val] == 0){
		return;
	}
	if ([treemap depth] == maxDepthToDraw){
		//draw aggregates
			...
			wxColour grayColor = wxColour (wxT("#c0c0c0"));
			this->drawDotNode (child, 0, brush, grayColor, dc);
		}
...
void DotDraw::recreateResourcesGraph()
{
	if (![filter checkForDotHierarchy]){
	        NSLog (@"It is not a Dot trace file");
		return;
	}
